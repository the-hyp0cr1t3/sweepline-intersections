...
auto sweepline::find_intersections(
  ...
  detail::enable_color = enable_color;
  ...
  for(size_t i = 0; i + 1 < vertical_segs.size(); i++) {
    if(std::fabs(vertical_segs[i].q.y - vertical_segs[i + 1].p.y) < geometry::EPS) {
      sweepline::intersection_t it { vertical_segs[i].q,
                std::vector<size_t>{ vertical_segs[i].seg_id, vertical_segs[i + 1].seg_id } };
      result.emplace_back(it);

      if(verbose)
        detail::debug_intersection(it, "vertical<->vertical");
    }
  }
  ...
  while(!event_queue.empty()) {
    ...
    while(vert_idx < vertical_segs.size()
      and vertical_segs[vert_idx].p.x <= top.p.x + geometry::EPS) {
        ...
        while(itr != seg_ordering.end()) {
          ...
          sweepline::intersection_t it { geometry::point_t{ sweepline::sweeplineX, it_y },
                    std::vector<size_t>{ itr->seg_id, vseg.seg_id } };

          if(verbose)
            detail::debug_intersection(it, "vertical<->non-vertical");
    ...
    if(verbose) {
      std::cerr << detail::format_heading_text("sweeplineX") << " = " << sweepline::sweeplineX << std::endl << std::endl;
    ...
    if(active[0].size() + active[1].size() + active[2].size() > 1) {
      ...
      result.emplace_back(sweepline::intersection_t{ top.p, active[0] });

      if(verbose) {
        std::cerr << detail::format_neutral_text("\nFound intersection!\n");
        std::cerr << detail::format_intersection(result.back()) << std::endl;
      }
    }
  ...
  std::vector<sweepline::intersection_t> answer;
  for(size_t i = 0, j; i < result.size(); i = j) {
    j = i;
    ...
    answer.emplace_back(sweepline::intersection_t{
                result[i].pt,
